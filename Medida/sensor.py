...
class Voltage_divider():
    ...
    def mean_value(self, vin):
        ...
        """
        self.logger.debug(f"Calculating mean value for {type(vin)}")
        if type(vin) == int or type(vin) == float or type(vin) == np.ndarray:
            self.meanV = vin*self.__r_value
            self.logger.debug(f"Mean value calculated: {self.meanV[0:int(0.01*len(self.meanV))+1]}")
        else:
            self.logger.error(f"Type of {type(vin)} is not valid")
            raise TypeError(
                "The type of argument accepted are float or numpy array")
        return self.meanV
    ...
    def noise(self, vin, mode='most probably'):
        ...
        """
        self.logger.debug(f"Calculating noise for {type(vin)} with mode {mode}")
        # The uncertainty is standard deviation
        if type(vin) == int or type(vin) == float:
            mu, sigma = self.__r_value, self.__uncertainty_r(
                mode)  # mean and standard deviation
            self.uncertaintyV = vin*np.random.normal(mu, sigma, 1)[0]
            self.logger.debug(f"Noise calculated: {self.uncertaintyV}")
        elif type(vin) == np.ndarray:
            n = len(vin)
            mu, sigma = self.__r_value, self.__uncertainty_r(
                mode)  # mean and standard deviation
            self.uncertaintyV = vin*np.random.normal(mu, sigma, n)
            self.logger.debug(f"Noise calculated: {self.uncertaintyV[0:int(0.01*len(self.uncertaintyV))+1]}")
        else:
            self.logger.error(f"Type of {type(vin)} is not valid")
            raise TypeError(
                "The type of argument accepted are int, float or numpy array")
        return self.uncertaintyV
    ...
    def __uncertainty_r(self, mode='most probably'):
        ...
        Returns:
            _type_: _description_
        """
        terms = [self.__deltar2/self.r2, self.__deltar1 /
                 (self.r1+self.r2), self.__deltar2/(self.r1+self.r2)]
        ...
        if mode == 'pesimist':
            ...
            raise AttributeError(
                "The mode to calculate the uncertainty in not correctly defined")
        return solution
